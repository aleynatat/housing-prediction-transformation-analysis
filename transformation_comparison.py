...
# ==========================================
# 8. FEATURE TRANSFORMATION (BOX COX)
# ==========================================
...
# --- XGBoost with Transformation(Only y) ---
xgb_transformed2 = XGBRegressor(random_state=42)
xgb_transformed2.fit(X_train, y_train_transformed)

y_pred_xgb_trans_2 = xgb_transformed2.predict(X_test)
y_pred_xgb_original = inverse_boxcox(y_pred_xgb_trans_2, lambda_y)

print("\n----- XGBoost With Transformation (Only y) -----")
print(evaluate_model(y_test, y_pred_xgb_original))

# --- XGBoost with Transformation(X and y) ---
xgb_transformed3 = XGBRegressor(random_state=42)
xgb_transformed3.fit(X_train_transformed_df, y_train_transformed)

y_pred_xgb_trans_3 = xgb_transformed3.predict(X_test_transformed_df)
y_pred_xgb_original2 = inverse_boxcox(y_pred_xgb_trans_3, lambda_y)

print("\n----- XGBoost With Transformation (X and y) -----")
print(evaluate_model(y_test, y_pred_xgb_original2))


# --- XGBoost With Hyperparameter Tuning and Transformation ---
xgboost_params = {
    "learning_rate": [0.01, 0.1, 0.2, 0.5],
    "max_depth": [3, 4, 5, 6, 7, 8, 10],
    "n_estimators": [100, 200, 300, 400, 500],
...
from sklearn.model_selection import RandomizedSearchCV
randomcv_final = RandomizedSearchCV(estimator=XGBRegressor(random_state=42),
                              param_distributions=xgboost_params,
                              cv=5, n_jobs=-1, verbose=1)
randomcv_final.fit(X_train_transformed_df, y_train_transformed)

print(f"\nBest XGBoost Params: {randomcv_final.best_params_}")

y_pred_xgb_tuned_final = randomcv_final.predict(X_test_transformed_df)
y_pred_xgb_original_final = inverse_boxcox(y_pred_xgb_tuned_final, lambda_y)

print("\n----- XGBoost With Hyperparameter Tuning and Transformation -----")
print(evaluate_model(y_test, y_pred_xgb_original_final))
...
best_params_lgbm = randomcv2.best_params_
...
# --- LightGBM with Transformation(Only y) ---
lgbm_transformed2 = LGBMRegressor(verbose=-1, random_state=42)
lgbm_transformed2.fit(X_train, y_train_transformed)

y_pred_lgbm_trans_2 = lgbm_transformed2.predict(X_test)
y_pred_lgbm_original = inverse_boxcox(y_pred_lgbm_trans_2, lambda_y)

print("\n----- LightGBM With Transformation (Only y) -----")
print(evaluate_model(y_test, y_pred_lgbm_original))


# --- LightGBM with Transformation(X and y) ---
lgbm_transformed3 = LGBMRegressor(verbose=-1, random_state=42)
lgbm_transformed3.fit(X_train_transformed_df, y_train_transformed)

y_pred_lgbm_trans_3 = lgbm_transformed3.predict(X_test_transformed_df)
y_pred_lgbm_original2 = inverse_boxcox(y_pred_lgbm_trans_3, lambda_y)

print("\n----- LightGBM With Transformation (X and y) -----")
print(evaluate_model(y_test, y_pred_lgbm_original2))

# --- LightGBM With Hyperparameter Tuning and Transformation ---
lgbm_params = {
    'learning_rate': [0.01, 0.03, 0.05, 0.1, 0.3],
    'max_depth': [3, 4, 5, 6, 7, -1],
    'min_child_samples': [10, 20, 30],
...
randomcv2_final = RandomizedSearchCV(estimator=LGBMRegressor(verbose=-1, random_state=42),
                               param_distributions=lgbm_params,
                               cv=5, n_jobs=-1, verbose=1)
randomcv2_final.fit(X_train_transformed_df, y_train_transformed)

print(f"\nBest LightGBM Params: {randomcv2_final.best_params_}")
best_params_lgbm_final = randomcv2_final.best_params_

y_pred_lgbm_tuned_final = randomcv2_final.predict(X_test_transformed_df)
y_pred_lgbm_original_final = inverse_boxcox(y_pred_lgbm_tuned_final, lambda_y)
print("\n----- LightGBM With Hyperparameter Tuning and Transformation -----")
print(evaluate_model(y_test, y_pred_lgbm_original_final))
